...
import type {ReportsSplitNavigatorParamList, SearchFullscreenNavigatorParamList, SearchReportParamList, TransactionDuplicateNavigatorParamList} from './Navigation/types';
...
const getReviewNavigationRoute = (
    route:
        | PlatformStackRouteProp<TransactionDuplicateNavigatorParamList, typeof SCREENS.TRANSACTION_DUPLICATE.REVIEW>
    ...
    let threadReportID = '';
    if ('threadReportID' in route.params) {
        threadReportID = route.params.threadReportID;
    } else if ('reportID' in route.params) {
        threadReportID = route.params.reportID;
    }
